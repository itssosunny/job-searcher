...
def collect(query: str | None, limit: int, category: str | None = None, timeout: int = 25) -> list[JobPosting]:
    ...
    for item in soup.find_all("item"):
        ...
        # WWR auto-expires listings ~30 days out; <expires_at> is a real per-item
        # closing date (RFC822). Store verbatim; _derive_status normalizes it.
        expires = item.find("expires_at")
        dl_text = expires.text.strip() if expires and expires.text else None
